...
import { validateDeckPortalUrl } from './deck-import/portal-url';
...
function Advanced() {
  const [method, setMethod] = useState<'url'|'list'>('url'); const [text, setText] = useState(''); const [message, setMessage] = useState('');
  const [step, setStep] = useState(1);
  const importDeck = () => { try { if (method === 'url') { validateDeckPortalUrl(text); setMessage('URLの形式を確認しました。復号規則の確定後にカード一覧へ展開します。'); } else { const lines = text.trim().split(/\n+/).filter(Boolean); if (!lines.length) throw new Error('カード名と枚数を貼り付けてください。'); setMessage(`${lines.length}行を読み込みました。カードマスター照合は公開データ更新後に行います。`); } setStep(2); } catch (error) { setMessage(error instanceof Error ? error.message : '読み込みに失敗しました。'); } };
